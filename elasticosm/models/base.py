...
class BaseElasticModel(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        origin = registry.instance_registry.get(
                                        self.__get_elastic_type_name__(),
                                        None
                                        )
        if origin is not None:
            ...
            if self.__class__ != BaseElasticModel:
                ...
                for attr_name, attribute_value in self.__class__.__dict__.items():
                    # skip the built-in stuff, db/elastic fields
                    # should never be called __something or _something
    ...
    def __getattribute__(self, name, *args, **kwargs):
        ...
        ret_val = super(BaseElasticModel, self).__getattribute__(
                                                name, *args, **kwargs)
    ...
    @classmethod
    def from_elastic_dict(cls, elastic_dict):
        obj_dict = elastic_dict['_source']
        from elasticosm.models.registry import ModelRegistry
        registry = ModelRegistry()
